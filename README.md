...
## **直接上手教程**
**点击.sln调试，直接查看 guide_docs/调试流程.md 跟着来即可**
...
- **操作系统**：Windows 10/11
- **开发工具**：Visual Studio 2022 (安装 C++ 桌面开发组件)
- **编译配置**：`Debug | x64` (推荐) 或 `Release | x64`
- **依赖**：无第三方库依赖，全部使用原生 Win32/MFC API。

## 🚀 快速上手

1.  **编译运行**：用 VS2022 打开 `智能机械臂.sln`，按 `F5` 运行。
2.  **开启诊断**：主界面点击 **“诊断(D)...”** 进入调试中心。
3.  **详细调试指南**：请参考 [调试流程详解 (新手必读)](guide_docs/调试流程.md)。
4.  **无硬件调试**：在 `Serial` 页勾选“使用模拟串口”，点击“连接”，即可发送指令并观察模拟回包。
4.  **导入参数**：如果你是第一次拉取本项目，请点击主界面的 **“导入参数...”** 并选择仓库中的 `.ini` 文件，以获得最新的标定结果。
...
- `guide_docs/`: [详细的调试流程指南](guide_docs/调试流程.md)。
- `progress/`: 开发阶段成果记录。
