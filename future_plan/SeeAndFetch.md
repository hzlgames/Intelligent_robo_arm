...
#### 阶段 2.5：平面坐标确定与“遮挡后导航”验证（下一步）
- **目标**：完成“目标点 → 桌面平面坐标”的稳定映射，并在抓取后即使目标被物体遮挡/相机短时看不到目标，也能依靠已确定的平面坐标继续运动到目标平面位置（导航到放置/对准点）。
- **关键思路**：
  - **平面模型**：明确桌面平面方程或等效的“像素到平面”的映射（Homography 或 Ray-Plane 相交）。
  - **坐标系约定**：建立统一的 **桌面平面坐标系 (X_plane, Y_plane)**，并定义其与 Base 坐标系的关系（可先用简化外参/标定流程验证链路）。
  - **遮挡鲁棒性**：在“锁定时刻”缓存目标的平面坐标与置信度；后续若视觉丢失，控制器切换为“按平面坐标导航”而不是依赖实时像素观测。
- **推荐实现（Pos0 / Pos1 双次居中自举，适配“遮挡后导航”）**：
  - **前提**：终点在桌面平面 \(Z=0\) 上；且“居中时相机明显朝下”（\(d_z<0\)），射线与平面求交稳定。
  - **Step A：确定抓取目标**：Point→锁定→Pinch 确认（只要得到“已确认抓取目标”即可）。
  - **Step B：目标居中并记录 Pos0**：
    - 将 **J5 回中值（500）** 使镜头“正”（J5 会改变相机朝向，回中能简化几何）。
    - 视觉伺服将“抓取目标”移到画面中心（CenterTarget/LookAndMove）。
    - 记录此时姿态 **Pos0**（至少包含末端/相机在 Base 下的 \((x,y,z,pitch)\) 与 **J1(Base Yaw)**；如可行也记录 J5 便于后续扩展）。
  - **Step C：终点居中并记录 Pos1（指尖末端）**：
    - 视线跟随手移动；用户指向终点并确认。
    - 将“食指末端”移到画面中心，记录姿态 **Pos1**（同上字段；同样建议 J5=500 保持镜头正）。
  - **Step D：由 Pos1 解算终点桌面坐标**：
    - 以 Pos1 得到相机中心 \(C=(C_x,C_y,C_z)\) 与光轴方向 \(d=(d_x,d_y,d_z)\)（由 J1/yaw + pitch 决定；\(d_z<0\)）。
    - 射线与桌面平面求交：\(t=-C_z/d_z\)，终点 \(P=C+t d\Rightarrow(x_g,y_g,0)\)。
    - 备注：相机相对末端存在固定偏置（如 \(L_{cam}\)），可先作为常量补偿/后续标定优化。
  - **Step E：回到 Pos0 并执行抓取**：
    - 机械臂回到 Pos0（目标应接近画面中心），再次锁定目标并完成抓取。
  - **Step F：盲移放置（遮挡也能走完）**：
    - 抓取后进入盲移：移动到 \((x_g,y_g,z_{safe})\)（终点上方一点，避免过早碰桌面），再开爪放置。
- **调试目标**：
  - **平面坐标输出可视化**：在 HUD 显示 `planeX/planeY`（单位 mm 或归一化），并能在日志中记录“锁定时刻/丢失时刻”的平面坐标证据。
  - **一致性测试**：同一目标在不同视角/位置下点击或检测到的 `planeX/planeY` 应稳定（误差可量化）。
  - **遮挡测试用例**：锁定目标 → 手/物体遮挡相机 1~3 秒 → 仍能持续向目标平面位置推进/对准（不要求抓取，只验证导航）。
- **验收**：
  - 能在无机械臂/模拟模式下复现：输出平面坐标稳定、遮挡后仍持续导航。
  - 有可复现证据：日志中能解释“何时锁定、锁定的平面坐标是多少、遮挡期间如何继续控制、何时恢复视觉”。
