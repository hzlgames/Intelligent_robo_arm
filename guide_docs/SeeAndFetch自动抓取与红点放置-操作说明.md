...
### 1.2 手势点选（SelectGoal / SelectTerminal）
See&Fetch 现在的默认流程会在真正抓取前，先让你通过手势确定：
- **要抓取的物体（SelectGoal）**：只允许 **Detector 候选框**
- **要放置的终点红点（SelectTerminal）**：指向桌面红点并确认

手势规则（来自 HandLandmarks）：
- **Point（伸食指）**：进入“搜索/锁定”流程
- **Pinch（捏合）保持一段时间**：确认（Confirmed）
- **OpenPalm（张手）保持一段时间**：取消

你在画面里会看到一个选中框（蓝框/提示框）随指向变化：
- 指向同一个候选连续稳定一段时间 → 从 Searching 进入 Locked
- 继续 Pinch 保持 → Confirmed

> 注意：如果你发现“手往左下，机械臂往右上”这类反向问题，先看第 3.3 节“方向取反”。
...
- **距离/到位判断**：支持 ArUco 深度或 bbox 面积（见第 4 节）。
 - **手势点选锁定**：SelectGoal 阶段会调用 Detector 的候选框，并且**只允许 Detector 候选**（不会用轮廓兜底）。
...
- **放置到位判断**：推荐使用 bbox 面积（红点越近，框面积通常越大）。
 - **终点锁定**：SelectTerminal 阶段会“跟随手势指向红点”确认，并记录 `terminal_pos`（姿态快照）。后续放置会先回到 `terminal_pos`，避免终点乱跳。
...
### 3.3 方向取反（相机翻转/坐标系相反时的最快修正）
现象：你的手/目标往画面某方向移动，但机械臂末端却向**相反方向**移动（常见于相机旋转 180° 或镜像设置不一致）。

处理：在 See&Fetch 诊断页底部 **“方向取反(相机翻转修正)”** 勾选对应项：
- **J1(左右)**：左右反向
- **J4(上下)**：上下反向（优先关节）
- **J3(上下)**：上下反向（J4 超出优先范围时会用 J3）
- **J2推进**：接近目标方向反向（越走越远时用）
- **J2下降**：放置下降方向反向（越降越抬时用）

> 说明：这些勾选会自动写入对应的 `Sign...` 参数（±1）。一般优先先勾 J1/J4/J3，让“居中跟随”先正确。
...
> 新增：如果你在 SelectTerminal 阶段已经手势确认过终点红点，系统会记录 `terminal_pos`，抓取后会先回到 `terminal_pos` 再进入 Place（减少“红点识别抖动导致终点乱窜”）。
...
### 7.3 下降方向反了
- 修改 **SignJ2Down**（+1 ↔ -1），或直接在诊断页底部勾选 **J2下降**（方向取反）

### 7.4 跟随方向反了（手往左下，机械臂往右上）
- 在诊断页底部勾选：
  - **J1(左右)**（左右反向）
  - **J4(上下)** / **J3(上下)**（上下反向）
  - 并点击“保存并应用”
